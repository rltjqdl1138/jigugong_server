...
const Signin = async(req, res)=>{
    ...
    /*
     * 1. SELECT FROM DB *
     * 2. Authentication each platform *
    **/
    const name = '김기섭'

    const token = await jwt.code({id, platform, name}, 'account')
...
router.get('/', async (req, res) =>{
    const data = await db.duplicatedCheck('nickname', 'kkk')
    console.log(data)
})
